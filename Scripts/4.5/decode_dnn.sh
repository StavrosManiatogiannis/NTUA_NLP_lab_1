...
for f in $graphdir/HCLG.fst $data/feats.scp $treedirff/tree; do
  [ ! -f $f ] && echo "$0: no such file $f" && exit 1;
done

# Generate state counts; will be used as prior
$cmd $dir/log/class_count.log \
  ali-to-pdf $treedir/final.mdl "ark:gunzip -c $alidir/ali.*.gz |" ark:- \| \
    analyze-counts --binary=false ark:- $dir/class.counts || exit 1;

finalfeats="ark,s,cs:$featstring |"
$cmd JOB=1:$nj $dir/log/decode.JOB.log \
  latgen-faster-mapped --min-active=$min_active --max-active=$max_active --max-mem=$max_mem --beam=$beam --lattice-beam=$latbeam --acoustic-scale=$acwt --allow-partial=true --word-symbol-table=$graphdir/words.txt $treedir/final.mdl $graphdir/HCLG.fst "$finalfeats" "ark:|gzip -c > $dir/lat.JOB.gz"

# Copy the source model in order for scoring
cp $treedir/final.mdl $srcdir
